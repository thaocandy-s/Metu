<template>
    <div class="shadow-md p-3 rounded-md m-3">
      <h2 class="p-4 d-flex align-items-center text-black text-3xl font-semibold">
        ...
        <span class="ml-2 text-2xl text-black">Bộ lọc hình dạng</span>
      </h2>
      <a-form layout="vertical" class="mb-4">
        ...
        <a-form-item class="text-center">
          <a-input class="w-100" :value="params.search" @input="onChange('search', $event)"
            placeholder="Mã hoặc tên hoặc tên lớp của hình dạng" allowClear />
        </a-form-item>
      </a-form>
    </div>
  </template>
  ...
  type ParamsFilterShape = {
    search: string;
  };
  
  const params = ref<ParamsFilterShape>({
    search: '',
  });
  ...
  function onChange(key: keyof ParamsFilterShape, event: Event) {
    if (
      event &&
      event.target &&
