  ...
  <div class="shadow-xl p-3 m-3 rounded-md flex h-full flex-col overflow-auto">
    <div class="flex justify-between items-center min-h-36">
      <h2 class="p-4 flex items-center text-primary text-3xl font-semibold">
        ...
        <span class="ml-2 text-2xl text-black">Danh sách hình dạng</span>
      </h2>
    ...
    <div class="flex h-0 flex-1 flex-col">
      <MetuTable wrapperClassName="min-h-[410px]" :columns="columnsShape" :data-source="dataSource" :loading="loading"
        ...
        <template #bodyCell="{ column, record }">
          <div v-if="column.key === 'action'" class="space-x-2 flex items-center justify-center">
            <a-tooltip title="Chỉnh sửa hình dạng" color="#1E90FF">
            ...
            <a-tooltip title="Thay đổi trạng thái hình dạng" color="#1E90FF">
              <a-button
                class="flex items-center justify-center"
                type="primary"
                size="large"
                @click="showConfirmChangeStatus(record.shapeId, record.shapeStatus)" 
                :icon="h(HeartOutlined)"
              />
            </a-tooltip>
          </div>

          <div v-else-if="column.key === 'shapeIconUrl'">
            <img :src="record.shapeIconUrl" alt="Icon" class="icon-image" />
          </div>

          <div v-else-if="column.key === 'shapeStatus'">
            <a-tag v-if="record.shapeStatus === 0" color="green" class="status-tag">Đang hoạt động</a-tag>
    ...
    <a-modal v-model:open="confirmChangeStatusVisible" title="Xác nhận thay đổi trạng thái" @cancel="confirmChangeStatusVisible = false"
      ...
      <p>Bạn có chắc chắn muốn thay đổi trạng thái hình dạng này không?</p>
    </a-modal>
  </div>
</template>
...
import { ShapeResponse } from "@/services/api/menu_style_config/shape.api";
import { EditOutlined, HeartOutlined } from "@ant-design/icons-vue";
import { h, ref } from "vue";

defineProps({
  dataSource: Array<ShapeResponse>,
  loading: Boolean,
...
const columnsShape = [
  ...
  { title: "Mã", dataIndex: "shapeCode", key: "shapeCode" },
  { title: "Tên", dataIndex: "shapeName", key: "shapeName" },
  { title: "ClassName", dataIndex: "shapeClassName", key: "shapeClassName" },
  { title: "Icon_URL", dataIndex: "shapeIconUrl", key: "shapeIconUrl" },
  { title: "Trạng thái", dataIndex: "shapeStatus", key: "shapeStatus" },
  { title: "Mô Tả", dataIndex: "shapeDescription", key: "shapeDescription" },
